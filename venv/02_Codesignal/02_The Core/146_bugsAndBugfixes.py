...
def bugsAndBugfixes(rules):
    pattern = r'(\d*)d(\d+)([+-]?\d+)?'  # r'([\d]*(?=d))d([\d](?<=d)*)([+]*[-]*[\d]+)*'
...
rs = "Roll d6-3 and 4d4+3 to pick a weapon, and finish the boss with 3d7!"
print(bugsAndBugfixes(rs))
rs = "d6-almost 01d4+2 12d01-3 5d5-00 a valid formula"
print(bugsAndBugfixes(rs))
'''
...
To this basic notation, an additive modifier can be appended that yields expressions in the form AdX+B 
or AdX-B. B is a number added to the sum of the rolls (or subtracted from it). 
...
(i.e. A and X are always positive in a formula-like substring), 
and any two substrings that may be formulas do not overlap.
