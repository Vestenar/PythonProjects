...
if __name__ == '__main__':                                          # логика самотестирования
    dolist = 1
    ...
    def selftest(testmask):
        if testmask & dolist:
            ...
            print(f'Visited {visitor.fcount:d} files and {visitor.dcount:d} dirs')
        if testmask & dosearch:
            visitor = SearchVisitor(sys.argv[3], trace=0)
            visitor.run(sys.argv[2])
            print(f'Found in {visitor.scount:d} files, visited {visitor.fcount:d}')
