def metroCard(lastNumberOfDays):
    ...
    for i in range(len(month)):
        if month[i] == lastNumberOfDays and month[(i+1)%len(month)] not in ans:
            ans.append(month[(i+1)%len(month)])
    return ans
...
the system allocates you a 31-day pass, which equals the number of days in January.
...
All of them have exactly 31 days, which means that you will definitely 
