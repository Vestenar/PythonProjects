...
def timedReading(maxLength, text):
    ...
    return len(re.findall(r'\b[a-z]{1,%d}\b'%maxLength, text, flags=re.IGNORECASE))

text = "The Fox asked the stork, 'How is the soup?'"
ml = 4
print(timedReading(ml, text))
'''
...
such that characters to the left of the leftmost letter and to the right of the 
