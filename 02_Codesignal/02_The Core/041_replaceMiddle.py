def replaceMiddle(arr):
    ...
    if l < 2:
        ...
        if l%2 == 0:
            arr[(l//2)-1:l//2+1] = [arr[(l//2)-1] + arr[(l//2)]]
    return arr
...
We define the middle of the array arr as follows:
    ...
    if arr contains an odd number of elements, its middle is 
the element whose index number is the same when counting from 
the beginning of the array and from its end;
    if arr contains an even number of elements, its middle is 
the sum of the two elements whose index numbers when counting 
