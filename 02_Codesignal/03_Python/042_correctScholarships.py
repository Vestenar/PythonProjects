def correctScholarships(bestStudents, scholarships, allStudents):
    return (set(bestStudents).issubset(set(scholarships)) and set(scholarships).issubset(set(allStudents)) and
            len(bestStudents) < len(scholarships) < len(allStudents))
bestStudents = [3, 5]
scholarships = [3, 5, 7]
allStudents = [1, 2, 3, 4, 5, 6, 7]
print(correctScholarships(bestStudents,scholarships,allStudents))
'''
For the upcoming academic year the Coolcoders University should decide which students 
will get the scholarships. Scholarships are considered to be correctly distributed 
...
You are given lists of unique student ids 
bestStudents, scholarships and allStudents, 

representing ids of the best students, students that will get a scholarship 
and all the students in the university, respectively. 
