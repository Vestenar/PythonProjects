...
class TestHTMLExporter(ExportersTestsBase):
    ...
    def test_png_metadata(self):
        ...
        check_for_png = re.compile(r'<comics src="[^"]*?"([^>]*?)>')
    ...
    def test_attachments(self):
        ...
        check_for_png = re.compile(r'<comics src="[^"]*?"([^>]*?)>')
        result = check_for_png.search(output)
        self.assertTrue(result.group(0).strip().startswith('<comics src="data:image/png;base64,iVBOR'))
        self.assertTrue(result.group(1).strip().startswith('alt="image.png"'))

        check_for_data = re.compile(r'<comics src="(?P<url>[^"]*?)"')
